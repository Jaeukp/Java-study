...
class game_Frame extends JFrame implements KeyListener, Runnable{
    ...
    int x, y;
    int[] cx ={0, 0, 0}; // 배경 스크롤 속도 제어용 변수 int bx = 0; // 전체 배경 스크롤 용 변수
    ...
    // 유저의 캐릭터가 움직이는 속도를 조절할 변수 int missile_Speed; // 미사일이 날라가는 속도 조절할 변수 int fire_Speed; // 미사일 연사 속도 조절 변수 int enemy_speed; // 적 이동 속도 설정 int player_Status = 0;
// 유저 캐릭터 상태 체크 변수 0 : 평상시, 1: 미사일발사, 2: 충돌 int game_Score; // 게임 점수 계산 int player_Hitpoint; // 플레이어 캐릭터의 체력

//int e_w, e_h; //소스 변경으로 인해 해당 변수가 필요없어졌습니다 //int m_w, m_h;
...
//다수의 폭발 이펙트를 처리하기 위한 배열
    ...
    public void init() {
        ...
        Missile_img = new ImageIcon("C:\\Users\\tjoeun\\IdeaProjects\\java-study\\src\\Solving\\Sutting\\Missile.png").getImage();
        Enemy_img = new ImageIcon("C:\\Users\\tjoeun\\IdeaProjects\\java-study\\src\\Solving\\Sutting\\enemy.png").getImage();
//이미지 만드는 방식을 ImageIcon으로 변경.

        Player_img = new Image[5];
        for (int i = 0; i < Player_img.length; ++i) {
            Player_img[i] =
                    new ImageIcon("C:\\Users\\tjoeun\\IdeaProjects\\java-study\\src\\Solving\\Sutting\\f15k_" + i + ".png").getImage();
        }
//플레이어 애니메이션 표현을 위해 파일이름을
//넘버마다 나눠 배열로 담는다.

        BackGround_img = new ImageIcon("C:\\Users\\tjoeun\\IdeaProjects\\java-study\\src\\Solving\\Sutting\\background.png").getImage();
//전체 배경화면 이미지를 받습니다.

        Cloud_img = new Image[3];
        for (int i = 0; i < Cloud_img.length; ++i) {
            Cloud_img[i] =
                    new ImageIcon("C:\\Users\\tjoeun\\IdeaProjects\\java-study\\src\\Solving\\Sutting\\cloud_" + i + ".png").getImage();
        }
//구름을 3개 동시에 그리는데 편의상 배열로 3개를 동시에 받는다.
        ...
        for (int i = 0; i < Explo_img.length; ++i) {
            Explo_img[i] =
                    new ImageIcon("C:\\Users\\tjoeun\\IdeaProjects\\java-study\\src\\Solving\\Sutting\\explo_" + i + ".png").getImage();
        }
...
//기본 값이 0인 bx가 -3500 보다 크면 실행
            ...
            buffg.drawImage(BackGround_img, (int) bx, 300, this);
            bx -= 1;
